...
- 直装版额外填写 `.app.tar.gz` 地址、`.sig` 文件的完整 base64 内容（不是文件名）、SHA-256 和包大小；服务端会校验签名格式。
- 先以小比例灰度，确认后编辑暂停记录并重新发布更高版本；已发布记录允许直接编辑元数据（如更正录入错误的更新签名），变更写入审计并对客户端下一次检查更新即时生效。
...
- 上传 DMG 供首次安装，上传 `.app.tar.gz` 供应用内更新；`.app.tar.gz.sig` 必须与 tar.gz 作为成对资产一起上传到发布存储（GitHub Release），保存 `.sig`、SHA-256 和字节数供运营录入。
...
1. 把 DMG 上传到版本化、不可被同名覆盖的 HTTPS 路径；把 `.app.tar.gz` 与 `.app.tar.gz.sig` 作为成对资产上传到 GitHub Release（tag `vX.Y.Z`），上传后核对资产清单与散列。
2. 用 `shasum -a 256` 复核 updater 归档，并读取实际字节数。
3. 在 mb-ops 的“内容运营 / 软件版本”创建 `direct` 草稿。
4. 粘贴同一归档的 URL、SHA-256、字节数和 `.sig` 文件的完整 base64 内容（`base64 -i <file>.sig` 的输出，不得填写文件名；服务端会拒绝非法值）。
