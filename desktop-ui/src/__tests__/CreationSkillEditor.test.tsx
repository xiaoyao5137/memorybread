...
describe('沉淀技能', () => {
  ...
  it('展示百分比分析进度，并自动保存为未安装草稿后由用户完成保存', async () => {
    ...
    expect(savedBodies[0].execution_steps[0]).not.toHaveProperty('detail_policy')
    expect(screen.queryByText('将独立组件展开为细节章节')).not.toBeInTheDocument()
    expect(screen.queryByText('最低组件覆盖率（%）')).not.toBeInTheDocument()
