...
- `enabled_tools` 是新增的可选请求字段；旧客户端不传时，Core Engine 与 Sidecar 补齐四个必备 Tool，新版客户端与 Sidecar 还默认启用官方内置的 `mermaid_diagram`。
...
- `chapter_design_agent` 同时输出 `creation.visual-plan.v1`：它按章节内容中的对象关系、步骤分支、时间交互、状态变化或实体关系判断是否需要代码图示，不按某一种方案名称写死规则。Harness 在章节设计完成后为每个有效 `diagram_spec` 调度一次章节级 Mermaid 准备步骤，Writer 必须把图放入 `section_title` 对应章节；质检按计划逐章检查，不再以“全文存在任意一张图”视为通过。
- 可选 Tool 当前包括 `plantuml_diagram`、`mermaid_diagram`、`github_search`。Mermaid 为官方内置且默认启用，但用户可以关闭；其余可选 Tool 未安装或未开启时不得进入 Agent 计划。显式 Skill 仍以自身 `execution_steps` 为业务执行边界，只有声明的画图步骤才会运行。
